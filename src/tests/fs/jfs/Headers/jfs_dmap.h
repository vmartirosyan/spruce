...
#define	NOFREE		((__s8) -1)	/* no blocks free */
 ...
 */
#define	MAXL0SIZE	((__s64)1 << L2MAXL0SIZE)
#define	MAXL1SIZE	((__s64)1 << L2MAXL1SIZE)
#define	MAXL2SIZE	((__s64)1 << L2MAXL2SIZE)
 ...
 */
#define AGTOBLK(a,ip)	\
	((__s64)(a) << (JFS_SBI((ip)->i_sb)->bmap->db_agl2size))
...
struct dmaptree {
	...
	__s8 budmin;		/* 1: min l2 tree leaf value to combine */
	__s8 stree[TREESIZE];	/* TREESIZE: tree		*/
	__u8 pad[2];		/* 2: pad to word boundary	*/
};				/* - 360 -			*/
...
struct dmap {
	...
	__u8 pad[1672];		/* 1672: pad to 2048 bytes		*/
...
struct dmapctl {
	...
	__s8 budmin;		/* 1: minimum l2 tree leaf value	*/
	__s8 stree[CTLTREESIZE];	/* CTLTREESIZE: dmapctl tree	*/
	__u8 pad[2714];		/* 2714: pad to 4096		*/
};				/* - 4096 -			*/
typedef struct {
        int counter;
 } atomic_t;
...
struct dbmap_disk {
	...
	__s8 dn_maxfreebud;	/* 1: max free buddy system		*/
	__u8 pad[3007];		/* 3007: pad to 4096			*/
};				/* - 4096 -				*/

struct dbmap {
	__s64 dn_mapsize;		/* number of blocks in aggregate	*/
	__s64 dn_nfree;		/* num free blks in aggregate map	*/
	...
	__s64 dn_agfree[MAXAG];	/* per AG free count			*/
	__s64 dn_agsize;		/* num of blks per alloc group		*/
...
extern int dbFree(struct inode *ipbmap, __s64 blkno, __s64 nblocks);

extern int dbUpdatePMap(struct inode *ipbmap,
			int free, __s64 blkno, __s64 nblocks, struct tblock * tblk);

extern int dbNextAG(struct inode *ipbmap);

extern int dbAlloc(struct inode *ipbmap, __s64 hint, __s64 nblocks, __s64 * results);

extern int dbReAlloc(struct inode *ipbmap,
		     __s64 blkno, __s64 nblocks, __s64 addnblocks, __s64 * results);

extern int dbSync(struct inode *ipbmap);
extern int dbAllocBottomUp(struct inode *ip, __s64 blkno, __s64 nblocks);
extern int dbExtendFS(struct inode *ipbmap, __s64 blkno, __s64 nblocks);
extern void dbFinalizeBmap(struct inode *ipbmap);
extern __s64 dbMapFileSizeToMapSize(struct inode *ipbmap);
#endif				/* _H_JFS_DMAP */
